...
(define-method :draw ROUNDTANGLE (gob a-window)
  (let* ((update-vals (g-local-value gob :update-slots-values))
    ...
    (gem:draw-roundtangle a-window
			  (the fixnum (aref update-vals +roundt-left+))
			  (the fixnum (aref update-vals +roundt-top+))
			  (q-max 0 (the fixnum (aref update-vals +roundt-width+)))
			  (q-max 0 (the fixnum (aref update-vals +roundt-height+)))
			  r r
...
(defun point-in-roundtangle (x y left top right bottom radius)
  (declare (fixnum x y left top right bottom))
