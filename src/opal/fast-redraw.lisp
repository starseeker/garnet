...
(define-method :set-styles LINE (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    (setf (aref update-vals +line-lstyle+) line-style)))

(define-method :set-styles RECTANGLE (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    (setf (aref update-vals +rect-fstyle+) filling-style)
    (setf (aref update-vals +rect-lstyle+) line-style)))

(define-method :set-styles MULTIPOINT (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
    (setf (aref update-vals +multi-lstyle+) line-style)))

(define-method :set-styles POLYLINE (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
...
(define-method :set-styles TEXT (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
    (setf (aref update-vals +text-lstyle+) line-style)))

(define-method :set-styles BITMAP (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
    (setf (aref update-vals +bm-fstyle+) filling-style)))

(define-method :set-styles ARC (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
...
(define-method :set-styles OVAL (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
    (setf (aref update-vals +arc-fstyle+) filling-style)
...
(define-method :set-styles CIRCLE (obj line-style filling-style)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
    ;;    (setf update-vals (vector update-vals))
...
(declaim (type (simple-vector *) frr-update-vals))
...
(define-method :set-frr-bbox LINE (obj)
  (let* ((update-vals (g-local-value obj :update-slots-values))
	 ...
	 (height (+ (abs (- (or y1 0) (or y2 0)))
		    (* (if projecting-p 2 1) line-thickness))))
    (declare (type (simple-vector *) update-vals))
    (declare (fixnum x1 x2 y1 y2 line-thickness lt/2 left top width height))
    (set-frr-bbox-fn left top width height)))


(define-method :set-frr-bbox RECTANGLE (obj)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
...
(define-method :set-frr-bbox ROUNDTANGLE (obj)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
...
(define-method :set-frr-bbox MULTIPOINT (obj)
  (let* ((update-vals (g-local-value obj :update-slots-values))
	 ...
	 (height (do ((min-y 9999) (max-y 0)
		   ...
		   (declare (fixnum min-y max-y))
		   (setf min-y (q-min min-y (the fixnum (cadr point))))
		   (setf max-y (q-max max-y (the fixnum (cadr point)))))))
    (declare (type (simple-vector *) update-vals))
    (declare (fixnum line-thickness 2lt 4lt left top width height))
    (set-frr-bbox-fn left top width height)))


(define-method :set-frr-bbox TEXT (obj)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
...
(define-method :set-frr-bbox BITMAP (obj)
  (let ((update-vals (g-local-value obj :update-slots-values))
...
(define-method :set-frr-bbox ARC (obj)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
...
(define-method :set-frr-bbox CIRCLE (obj)
  (let ((update-vals (g-local-value obj :update-slots-values)))
    (declare (type (simple-vector *) update-vals))
